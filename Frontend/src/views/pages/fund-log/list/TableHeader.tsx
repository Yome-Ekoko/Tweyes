...
import { apiDownloadFundLogs } from 'src/services/FundLogService'
...
const TableHeader = (props: TableHeaderProps) => {
  ...
  const handleDownload = async () => {
    ...
    const response = await apiDownloadFundLogs({ startDate, endDate })
    setIsSubmitting(false)
    if (response?.data) {
      const filename = `fundlog-report-${new Date().toISOString()}.csv`
